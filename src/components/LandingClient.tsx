...
import { ArrowLeft, ArrowRight, Repeat, ShieldCheck, Lock, BookOpenText } from "lucide-react";
    ...
    <div className="min-h-dvh overflow-x-hidden">
      ...
      <header className="mx-auto max-w-6xl px-5 md:px-8 pt-5 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5">
          ...
          <span className="text-[11px] tracking-widest text-muted mt-1">AQIM</span>
        </Link>
      ...
      <section className="mx-auto max-w-3xl px-5 md:px-8 pt-10 md:pt-14 pb-12 text-center">
        ...
        {ayahArabic && (
          <>
            <p
              className="font-quran text-lg md:text-xl text-muted leading-[2.1] max-w-2xl mx-auto"
              dir="rtl"
            >
              {cleanAyah(ayahArabic)}
            </p>
            {lang === "en" && ayahTranslation && (
              <p className="text-sm text-muted/80 italic max-w-xl mx-auto mt-2">
                “{ayahTranslation}”
              </p>
            )}
          </>
        ...
        <h1 className="font-heading text-4xl md:text-5xl font-bold text-primary leading-[1.3] mt-7">
          {t("landing.slogan")}
        </h1>
      ...
      <section className="mx-auto max-w-6xl px-5 md:px-8 py-8 md:py-12">
        ...
        <div className="grid gap-3 md:grid-cols-3 md:items-stretch">
          <Feature Icon={Lock} title={t("landing.why1.title")} body={t("landing.why1.body")} />
          <Feature Icon={ShieldCheck} title={t("landing.why2.title")} body={t("landing.why2.body")} />
          <Feature Icon={Repeat} title={t("landing.why3.title")} body={t("landing.why3.body")} />
        </div>
      </section>
      ...
      <section className="mx-auto max-w-2xl px-5 py-6">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          <ModeChip Icon={MosqueIcon} label={t("mode.faraid")} hint={t("mode.faraid.hint")} />
          <ModeChip Icon={NafilahIcon} label={t("mode.nafl")} hint={t("mode.nafl.hint")} />
          <ModeChip Icon={QiyamIcon} label={t("mode.qiyam")} hint={t("mode.qiyam.hint")} />
        </div>
      </section>
